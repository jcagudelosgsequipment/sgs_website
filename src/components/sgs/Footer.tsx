...
import { isRentalsHost, MAIN_DOMAIN_URL } from "@/utils/domain";
...
export const Footer = () => {
  ...
  const resolvedNavLinks = navLinks.map((link) => {
    if (link.external || !rentalsHost) return link;
    if (link.to === "/") return link;
    return { ...link, to: `${MAIN_DOMAIN_URL}${link.to}`, external: true };
  });
    ...
    <footer className="bg-slate-950 text-slate-300">
      <div className="container mx-auto px-4 py-16 lg:py-20">
        <div className="grid gap-12 lg:grid-cols-3 lg:gap-10">
          <div>
            {rentalsHost ? (
              <a href={MAIN_DOMAIN_URL} aria-label="SGS Equipment" className="inline-flex items-center">
                <img
                  src="/SGS_LOGO.webp"
                  alt="SGS Equipment"
